...
$\epsilon$ 代表的是「可以無限接近」理論極限。我們可以透過兩個情境來觀察：

*   **理想狀態 ($\epsilon = 0$)**：當符號出現機率剛好是 $2$ 的負整數次冪時（例如 $1/2, 1/4 \dots$），我們可以使用 Huffman 編碼達到 $L = H(X)$，此時沒有任何編碼浪費。
*   **現實狀態 ($\epsilon > 0$)**：當機率分佈不規則時（例如機率為 $0.6, 0.2, 0.2$），單個符號的編碼長度 $L$ 會大於 $H(X)$，產生的差值就是 $\epsilon$（編碼浪費）。

**如何縮小 $\epsilon$？**
香農的精髓在於：透過**「成組編碼」 (Block Coding)**，將多個符號看作一組來編碼。隨著組的大小（Block Size）增加，平均每個符號的編碼長度 $L$ 會越來越接近 $H(X)$，即 $\epsilon$ 會趨近於 $0$。
...
#### 情境一：理想狀態 ($\epsilon = 0$)
當符號出現機랜度剛好是 $2$ 的負整數次冪時：
*   **信源**：$\{A, B, C, D\}$，機率為 $\{0.5, 0.25, 0.125, 0.125\}$。
*   **熵 $H(X)$**：
    $$H(X) = -(0.5 \log_2 0.5 + 0.25 \log_2 0.25 + 0.125 \log_2 0.125 + 0.125 \log_2 0.125) = 1.75 \text{ bits}$$
*   **平均碼長 $L$**：使用 Huffman 編碼可得 $L = 1.75$。
*   **結果**：$L = H(X)$，即 $\epsilon = 0$。

#### 情境二：現實狀態 ($\epsilon > 0$)
當機率分佈不規則時：
*   **信源**：$\{A, B, C\}$，機率為 $\{0.6, 0.2, 0.2\}$。
*   **熵 $H(X)$**：
    $$H(X) \approx -(0.6 \log_2 0.6 + 0.2 \log_2 0.2 + 0.2 \log_2 0.2) \approx 1.37 \text{ bits}$$
*   **平均碼長 $L$**：使用固定長度編碼（每個符號需 2 bits），則 $L = 2$。
*   **結果**：$L > H(X)$，此時 $\epsilon = 2 - 1.37 = 0.63$。
...
在**有損壓縮**的情況下，香農引入了失真率函數 $R(D)$, 它描述了在允許一定失真 $D$ 的情況下，所需的最小編碼率 $R$。這反映了**資訊量與失真度之間的權衡 (Trade-off)**。
...
1.  **有效壓縮區 ($0 \le D \le \sigma^2$)**：
    *   **物理意義**：當你要求的誤差 $D$ 小於訊號本身的能量 $\sigma^2$ 時，你必須傳送資訊來描述訊號的細節。
    *   **數學邏輯**：當 $D$ 趨近於 $0$（追求無損）時，$R(D)$ 會趨於無窮大；當 $D = \sigma^2$ 時，$R(D) = 0$。
2.  **無資訊區 ($D > \sigma^2$)**：
    *   **物理意義**：如果允許的誤差已經大於訊號本身的波動，那麼你根本不需要傳送任何資訊。
    *   **數學邏輯**：此時 $R(D) = 0$，接收端只需要直接輸出一個常數（例如 $0$）即可滿足失真要求。

#### 2.3.2 具體實例說明

假設我們有一個標準高斯訊號，其變變異數 **$\sigma^2 = 1$**：

*   **案例 A：高精度需求 (低失真)**
    若要求失真度 $D = 0.0625$ (即 $1/16$)：
    $$R(0.0625) = \frac{1}{2}\log_2\left(\frac{1}{0.0625}\right) = \frac{1}{2}\log_2(16) = \frac{1}{2} \times 4 = 2 \text{ bits/sample}$$
    *解讀：為了極高的精度，每個樣本需要 2 個 bits。*

*   **案例 B：中等精度需求**
    若放寬要求，失失真度 $D = 0.25$ (即 $1/4$)：
    $$R(0.25) = \frac{1}{2}\log_2\left(\frac{1}{0.25}\right) = \frac{1}{2}\log_2(4) = \frac{1}{2} \times 2 = 1 \text{ bit/sample}$$
    *解讀：增加容忍度後，壓縮率大幅提升，僅需 1 個 bit。*

*   **案例 C：極低精度需求 (高失真)**
    若失真度 $D = 2$ (已大於 $\sigma^2$)：
    $$R(2) = 0 \text{ bits/sample}$$
    *解讀：不需要傳送任何數據，直接輸出 0 即可。*

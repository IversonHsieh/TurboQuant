...
- [視覺化流程圖](#視覺化流程圖)
- [什麼是 QJL？](#什麼是-qjl)
- [技術意義：QJL 與 PolarQuant 的互補關係](#技術意義qjl-與-polarquant-的互補關係)
...
<a id="技術意義qjl-與-polarquant-的互補關係"></a>

## 技術意義：QJL 與 PolarQuant 的互補關係
...
更形式化地，給定 $N$ 個點，存在一個隨機投影 $A \in \mathbb{R}^{k \times d}$，將 $d$ 維向量 $x \in \mathbb{R}^d$ 投影到 $k = O(\log N / \epsilon^2)$ 維，使得對所有點對 $(x, y)$ 有：

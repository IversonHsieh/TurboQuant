...
> 📝 **圖解說明**：
>
> **這張圖在畫什麼？** 圖分為三個部分：上方是三個迭代階段的示意（初始 → Iteration 1 → 收斂），中間是 MSE 收斂曲線，下方是演算法步驟流程。
>
> **❓ 第一次的 $c_i$（量化層級）是隨機選的嗎？** 不完全是。初始量化層級的選擇有幾種常見策略：
>
> 1. **均勻間距**（圖中所示）：將值域等分，例如在 $[-1, 1]$ 上取 $-0.75, -0.25, 0.25, 0.75$。這是最常見的初始方式，如圖中「初始 (Iteration 0)」所示。
> 2. **分位數初始化**：取分佈的等機率分位數作為初始層級，讓每個區間的機率大致相等。
> 3. **隨機初始化**：從分佈中隨機抽樣 $N$ 個點作為初始層級。這種方式可行，但收斂速度可能較慢，且可能收斂到不同的局部最佳解。
>
> 圖中使用的就是「均勻間距」初始化（標註為「均勻間距初始值」）。初始值的選擇會影響收斂速度和最終結果（對於多峰分佈可能收斂到不同的局部最佳解），但對於單峰分佈（如高斯分佈），通常都會收斂到同一個全域最佳解。
>
> **❓ 「收斂至最佳解」具體是什麼意思？要把什麼東西收斂成最佳解？** 要收斂的是**決策邊界 $\{b_i\}$ 和量化層級 $\{c_i\}$ 這兩組參數**，使它們同時滿足以下兩個最佳化條件：
>
> - **最近鄰條件**：每個邊界 $b_i$ 恰好位於相鄰兩個質心的中點，即 $b_i = \frac{c_i + c_{i+1}}{2}$
> - **質心條件**：每個質心 $c_i$ 恰好是其對應區間的條件期望值，即 $c_i = E[X \mid X \in R_i]$
>
> 當這兩組參數同時滿足上述條件時，就稱為**最佳解**——此時 MSE 無法再透過調整任何一組參數來降低。換句話說：
>
> - **收斂的對象**：決策邊界 $\{b_1, b_2, \ldots, b_{N-1}\}$ 和量化層級 $\{c_1, c_2, \ldots, c_N\}$
> - **收斂的判斷標準**：當迭代前後的量化層級變化量 $\max_i |c_i^{(k+1)} - c_i^{(k)}|$ 小於收斂閾值 $\varepsilon$ 時停止（如圖中 MSE 曲線下方的紅色虛線「收斂閾值 ε」）
> - **收斂的保證**：每次迭代 MSE 保證單調遞減（不會變大），最終收斂至局部最佳解；對於單峰分佈，局部最佳解即為全域最佳解
>
> 可以把整個過程想像成：一開始隨意畫幾條分割線和代表點（初始猜測），然後反覆執行「根據代表點調整分割線 → 根據分割線調整代表點」，直到分割線和代表點都不再移動為止——這時的配置就是最佳解。
...
> 💡 **用「全班考試成績」來理解這個公式**：
>
> 這個公式看起來很嚇人，但其實你早就會了——它就是在算**加權平均**。我們用一個日常的例子來理解：
>
> **場景**：全班 20 位同學考試，成績分佈如下：
>
> | 分數 $x$ | 得此分數的人數 |
> |---------|-------------|
> | 60 分 | 2 人 |
...
> **全班平均幾分？** 你一定會這樣算：
>
> $$\text{平均} = \frac{60 \times 2 + 70 \times 5 + 80 \times 8 + 90 \times 4 + 100 \times 1}{2 + 5 + 8 + 4 + 1} = \frac{1570}{20} = 78.5$$
>
> 拆開來看：
> - **分母** $2 + 5 + 8 + 4 + 1 = 20$：就是**總人數**
> - **分子** $60 \times 2 + 70 \times 5 + \cdots$：就是「每個分數 × 拿到該分數的人數」再加總——也就是**全班總分**
> - 兩者相除 = **平均分數**
>
> **為什麼分子要「分數 × 人數」？** 因為 70 分有 5 個人拿到，這 5 個人對總分的貢獻是 $70 \times 5 = 350$ 分。如果你只加人數（$2+5+8+4+1=20$），那只是總人數，不是總分。**「分數 × 人數」才能把「值」和「份量」結合起來**，算出真正的總分。
>
> **現在對應到公式**：
>
> | 考試成績的例子 | 質心條件公式 | 對應關係 |
> |--------------|------------|---------|
> | 分數 $x$ | $x$ | 我們關心的**值** |
> | 人數 | $f_X(x)\, dx$ | 該值出現的**份量**（機率密度 × 微小長度） |
> | 總人數 = 人數相加 | 分母 $\int f_X(x)\, dx$ | 該區間的**總機率**（= 面積） |
> | 總分 = 分數 × 人數 相加 | 分子 $\int x \cdot f_X(x)\, dx$ | 該區間的「值 × 份量」加總 |
> | 平均 = 總分 ÷ 總人數 | $c_i$ = 分子 ÷ 分母 | 該區間的**加權平均**（質心） |
>
> 關鍵對應：**「人數」對應到 $f_X(x)\, dx$**。在連續分佈中，$f_X(x)$ 是機率密度，$dx$ 是一小段長度，兩者相乘 $f_X(x)\, dx$ 就是「$X$ 落在 $x$ 附近那一小段的機率」——就像「拿到 70 分的人數」一樣，是那個值出現的份量。
>
> **所以 $x \cdot f_X(x)$ 是什麼意思？** 就像「70 分 × 5 人 = 350 分」一樣——**把「值」乘上「份量」**。在考試的例子中，這是「該分數對總分的貢獻」；在質心公式中，這是「該 $x$ 值對加權總和的貢獻」。人多的分數對平均的影響大，機率密度高的 $x$ 值對質心的影響大——完全一樣的道理。
>
> **那 $f_X(x)$ 本身是什麼？** $f_X(x)$ 是「機率密度」，對應到考試例子中就像是「每單位分數區間有幾人」。它本身不是機率（就像「每分 0.4 人」不是總人數），要乘上區間寬度 $dx$ 才是該小段的機率（就像「每分 0.4 人 × 10 分的區間 = 4 人」）。整個定義域積分起來必定等於 1（= 100%），就像全班人數加起來就是全部的學生。
>
> **物理類比**：把 $f_X(x)$ 想成一塊**密度不均勻的薄板**的密度分佈，那麼：
> - 分母 $\int f_X(x)\, dx$ = 薄板的**總質量**（對應：總人數）
> - 分子 $\int x \cdot f_X(x)\, dx$ = 薄板對原點的**力矩**（對應：總分）
> - 兩者相除 = 薄板的**重心位置**（對應：平均分數）
>
> 這就是為什麼 $c_i$ 被稱為「質心」（centroid）——它就是該區間在機率密度分佈下的**重心**。密度高的地方對重心的拉力大，所以 $c_i$ 會偏向 $f_X(x)$ 較高的那一側，而不是簡單的區間中點。

...
> 📝 **圖解說明**：圖中的鐘形曲線是機率密度函數 $f_X(x)$（此處以常態分佈為例）。黃色虛線 $b_1, b_2, b_3$ 是**決策邊界**，將 x 軸分成四個量化區域 $R_1 \sim R_4$。每個區域用不同顏色標示，其面積代表該區間的機率。紅色/青色/黃色/紫色圓點 $c_1 \sim c_4$ 是**量化層級（質心）**，它們是各自區域的**條件期望值** $c_i = E[X \mid X \in R_i]$，即該區域的「最佳代表值」，**不是面積**。任何落入 $R_i$ 的輸入值都會被量化為 $c_i$。
